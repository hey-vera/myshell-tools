...
- **/goal now emits a real plan doc**: Full proposal (vision + per-goal approach/rationale over alts + dependency phrases + checklist + heads-up) is printed *and* written to `PLAN.md` in cwd for external review/edit (exactly the "make a plan and get approval" flow Claude/GPT/Replit agents do). User still gets the in-chat selector (Start / partial / Edit-park) + /goals /todo for accept/decline/refine. Existing auto-stage "Staged N ... shall I start?" + parked-for-review paths unchanged. Updated help text. (Nice-to-have for one-chat-to-rule-them-all.)
...
- Pressure-smart UX everywhere: dynamic composer placeholder (high-p warns "keep it short"; context when @goals present); DAG banner calls out shedding; pressure passed through state/props.
- Final polish on auto default, decomposition feedback, and hints.
- Verified: build clean, UI+core tests pass, sequential path preserved.
- See scheduler-flag, menu, InputBox, App, menu-completion.
...
## 3.147.0
fix(ci): green the cross-platform CI matrix (Windows/macOS/Linux × Node 22/24) — two test-only fixes, no product change. Found by auditing why every recent `main` CI run was red across ALL operating systems. Two distinct, long-standing test defects (both pre-existing, surfaced by the owner's "must work on Windows and Linux" requirement): (1) **`test/unit/attachments.test.ts` was not Windows-portable** — it hardcoded POSIX path literals (`/work/real.png`, `/home/u/pics/y.bmp`) and compared them against the output of `resolveImageAttachments`, which uses `node:path` `resolve()`. On Windows `resolve('/work','real.png')` yields `\work\real.png`, so the injected `fileExists` set never matched and 4 assertions failed. The PRODUCT code was already correct (platform-aware `node:path`); the test now derives every expected/injected path via `resolve()`/the same base dirs, so it asserts against whatever the running OS actually produces (passes on Linux, macOS, AND Windows). (2) **`test/integration/menu-cli.test.ts` was non-hermetic** — its "no provider signed in → auth gate" assertion depended on the host's ambient auth state: it passed on a fresh CI runner but FAILED on a developer box / Replit container where providers are signed in (the spawned CLI skipped the gate and entered chat). Worse, a truly-zero-state machine launches the first-run *onboarding wizard* (not the menu), and the test's piped input drove that wizard into a real OAuth attempt that hung. Fixed by making the spawn fully hermetic: every provider credential-home env var (`CLAUDE_CONFIG_DIR`/`CODEX_HOME`/`GROK_HOME`/`XDG_CONFIG_HOME`/`XDG_DATA_HOME`/`HOME`/`USERPROFILE`) points at one empty temp dir (providers detect as installed-but-not-signed-in), the Replit-detection vars are cleared (so `replitPersistentEnv` can't redirect those homes back at the workspace's real creds), and a `{onboarded:true}` config is seeded in the isolated state dir — modeling a RETURNING user who simply isn't signed in, so the menu + auth gate renders deterministically on every host. All 4 menu-cli integration tests now pass regardless of ambient auth. Also re-audited the cross-platform product surface and confirmed it is sound: every `/dev/tty`/raw-mode/clipboard path is `win32`-guarded (Windows takes the `'inherit'`/`null` branch before any POSIX-only `openSync('/dev/tty')`), the Replit persistent-dir logic is a verified no-op off Replit (read-only `existsSync` checks; `loginPersistentEnv` hard-returns when `!isReplit`), and an off-Replit one-shot (`isReplit→false`) was verified working end-to-end. Full gate green: 6163 unit/arch pass, all integration pass, tsc/eslint/knip clean, lockfile firewall 0.

## 3.146.0
fix(routing): orchestration now spawns ONLY signed-in providers — closes the MIXED-auth edge that 3.145.0 explicitly deferred. Previously `cli.ts` passed `buildProviders(...)` (every INSTALLED provider) into `OrchestrateDeps`, so in a mixed setup (some providers signed in, others installed-but-signed-out) a preflight/route/work step could pick an unauthenticated CLI and stall on its blocking auth prompt. Fix: a new `buildAuthenticatedProviders(cwd, env)` filters `buildProviders` down to providers whose `env[id].authenticated` is true, and all three orchestration call sites in `cli.ts` (run, chat, and the menu chat entry) now use it. Detection still lists every INSTALLED provider for `doctor`/`login` (unchanged) — this only narrows the set `orchestrate` is allowed to spawn, so the no-providers guard fires cleanly when nothing is signed in instead of stalling on a signed-out CLI. Subscription-OAuth-safe: pure in-process filter on already-detected status, no new spawn/probe/network. Verified live in a real mixed setup: `myshell run` routes to the signed-in provider and returns a correct answer (exit 0) instead of hanging. Full gate green: 6163 unit/arch pass (+3: signed-out excluded, empty when none signed in, all-signed-in included), UI 128, all 4 PTY smokes, tsc/eslint/knip clean, lockfile firewall 0. Removes the KNOWN EDGE caveat from 3.145.0 — the plug-and-play path (install → auth your providers → chat) is now hardened for partial-auth setups too.

## 3.145.0
fix(ux): `myshell run` with no signed-in provider now gives fast, clear guidance instead of hanging. Found by walking the real flow: an impatient brand-new user who runs a one-shot task BEFORE signing in got a silent ~60s hang (myshell proceeded to attempt work against an unauthenticated provider CLI, which blocks). Added a fast auth pre-check at the `run` entry: if no provider is authenticated (`hasAuthenticatedProvider(env)` — opencode counts as signed-in only when it actually has creds), print "No providers are signed in. Run `myshell-tools login` …" and exit 1 — mirroring `doctor`'s "Not ready" message. Verified live: truly-zero-auth `run` now exits immediately with guidance; an authenticated `run` is unchanged (returns a correct answer). Gate green: 6160 unit pass, tsc/eslint/knip clean. KNOWN EDGE (not addressed here, surfaced for the owner): in a MIXED setup (some providers signed in, others installed-but-not-signed-in), a preflight/route step can still try an unauthenticated CLI and stall — fully fixing that is routing-layer surgery (filter unauthenticated providers out of every spawn path) and is deferred rather than risk destabilizing routing before acceptance testing; the normal flow (sign in your providers, then chat) is unaffected.

## 3.144.0
fix(auth): grok sign-in now persists across Replit sessions — plug-and-play parity with claude/codex/opencode. Found by walking the real flow: after a container reset, `myshell doctor` showed grok "not signed in" while the other three stayed authenticated. Root cause: claude/codex/opencode persist because their config-dir env vars (`CLAUDE_CONFIG_DIR` / `CODEX_HOME` / `XDG_*`) are set by the replit-tools wrapper, but nothing sets grok's `GROK_HOME`, so its `~/.grok` creds are ephemeral on Replit. Fix (grok supports `GROK_HOME` to override its config dir): (1) new `replitGrokHome(cwd)` → `.replit-tools/.grok-persistent`; (2) `loginPersistentEnv` now creates that dir + sets `GROK_HOME` when logging grok in (so `myshell login grok` writes creds to the persistent dir); (3) `replitPersistentEnv` redirects `GROK_HOME` to it once it exists; (4) `cli.ts` applies that redirect to `process.env` ONCE at startup (before any detection/spawn) so detection, the work spawn, and login all agree — mirroring how the wrapper handles the other three. No-op off Replit (the dir never exists → grok uses its normal `~/.grok`), and never overrides an already-set `GROK_HOME`. Verified: the real binary launches clean (`doctor` shows grok + "Ready"; `myshell run` returns a correct answer, exit 0). Gate green: 6160 unit pass (+3: GROK_HOME set when the dir exists, never-override, login creates the dir), tsc/eslint/knip clean.

## 3.143.0
fix(ux): surface `grok` in the two user-facing provider lists it was missing from — found by walking the real plug-and-play flow (install → `myshell --help` → auth → chat). (1) `myshell --help` listed the `login` providers as "(claude, codex, or opencode)" — now "(claude, codex, opencode, or grok)", and the `--code` note covers grok's device-code flow. (2) the "No providers are available" orchestrate error now says "(claude, codex, opencode, or grok)". (`myshell login grok` and grok detection already worked — these were stale help/error strings only.) Verified the end-to-end one-shot path works against a live provider: `myshell run "…"` launches → detects → routes → real work call → correct answer + token accounting, exit 0. No behavior change; pure user-facing strings. Gate green: 6157 unit pass, tsc/eslint clean.

## 3.142.0
refactor(quality): rank-10 guard governs blocking MODEL calls ONLY — it no longer sheds rank-9's local retrieval, so ranks 9 + 10 are safe to enable together. The flag-flip validation surfaced an over-aggressive interaction: with both `MYSHELL_REQUIRED_INVESTIGATION` and `MYSHELL_PREFLIGHT_GUARD` ON, the rank-10 overhead guard counted rank-9's `buildRetrievalContext` (a read-only LOCAL grep — no model call, no quota) against the blocking-model-call budget and shed it once the intent extractor consumed budget=1, suppressing local grounding on budget-constrained turns. Fix: the rank-9 retrieval is no longer wrapped by the guard or counted — rank 10 coordinates blocking MODEL calls only (the intent extraction + the upstream recap/understanding warmups), keeping the two features orthogonal. Net effect: enabling the overhead guard never weakens local investigation. Tests updated to assert the new behavior (rank-9 retrieval runs regardless of the guard; the guard sheds the intent MODEL call when over budget / at pressure 3). Default-OFF and byte-identical when off is unchanged. Full gate green: 6157 unit pass, UI 128, tsc/eslint/knip clean; and the all-audit-flags-ON validation now has no real-interaction failures (remaining diffs are tests pinned to a specific flag state, overridden by a global env override — not co-enablement defects). Removes the "hold rank 10" caveat from 3.141.0.

## 3.141.0
feat(quality): aggregate preflight-overhead guard (audit rank 10 — **the audit backlog is now fully cleared**), default-OFF and byte-identical when off. `capability-budget.ts` was explicitly ADVISORY ("nothing here enforces a call budget on the live chat path"; `MAX_ADDED_BLOCKING_CALLS=1` is an intent target). This makes that ceiling ENFORCED: behind a new default-OFF gate (`MYSHELL_PREFLIGHT_GUARD` ∈ {1,true,on,yes} OR `config.experimentalPreflightGuard`, resolved by `preflightOverheadGuardEnabled` — a verbatim mirror of the rank-7/8/9 flag helpers), `orchestrate` counts the BLOCKING pre-answer model calls actually taken this turn and SHEDS the next avoidable optional preflight when the count would exceed the turn-class budget. The pure core is `preflightAdmits(obs, turnClass)` (admits another optional blocking call iff `blockingCallsSoFar < CAPABILITY_BUDGET[turnClass].addedBlockingCalls` AND `pressure < 3`, where `decideShed` already kills the intent pass) + `turnClassOf(tier, risk)` (worker+low → trivial/budget 0; manager or high/critical → substantial; else normal). The counter is seeded from `observedBlockingCalls` (the interface layer's upstream resume-recap-refresh + understanding-warmup calls), incremented after each in-orchestrate blocking call (the unified extractor, the non-unified intent, the rank-9 retrieval), and consulted before each OPTIONAL preflight; when over budget the shed uses the EXACT existing fallbacks (the non-unified intent → `rulesIntentFrame` rules frame, no model call; the rank-9 retrieval → empty `investigationContext`) and the CORE answer is never shed. **No quota meter / subscription-safe**: reuses the EXISTING `governorPressure` cooldown signal (`?? pressureFromSignals({})`) + the `CAPABILITY_BUDGET` ceiling — NO new probe, token meter, model call, or network. **Neutral by default**: flag OFF ⇒ menu omits `preflightGuard`/`observedBlockingCalls`, every guard `if` is dead, the counter is computed but consulted by nothing, and the call sequence + prompts + event stream are byte-identical to today. Full gate green (conductor-verified): 6157 unit/arch pass (+33: `preflightAdmits` truth-table, the flag truth-table, and an orchestrate suite — OFF byte-identical call count, ON within/over budget shedding the intent pass + rank-9 retrieval while the core survives, trivial-turn budget-0, pressure-3 parity with `decideShed`, unified-path extractor counting + later-retrieval shed), UI 128, all 4 PTY smokes, tsc/eslint/knip clean, lockfile firewall 0. (The rank-10 implementation was committed in 6d7811a; this entry documents it + bumps the version after a full conductor verification — diff reviewed for OFF byte-identity + no churn, full gate re-run.) **With rank 10 the prioritized audit backlog (P0–P2, ranks 1–10) is complete** — all behind default-OFF experimental gates pending the owner's acceptance test.

## 3.140.0
feat(quality): enforced local-investigation directive (audit rank 9), default-OFF and byte-identical when off. The audit found local retrieval only fires when the confidence brain's own logic chooses to investigate — so an explicit inspect-shaped turn the brain happens to score "confident enough" can reach the work call with no grounding. Behind a new default-OFF gate (`MYSHELL_REQUIRED_INVESTIGATION` ∈ {1,true,on,yes} OR `config.experimentalRequiredInvestigation`, resolved by `preflightRequiredInvestigationEnabled` — a verbatim mirror of rank-7/rank-8's flag helpers), the turn directive now carries `requiredInvestigation: 'local'|'web'|'none'` (derived purely by `deriveRequiredInvestigation`: `'local'` ONLY when the engagement plan scheduled an `INVESTIGATE_CONTEXT` action AND a repo is present; `'web'` reserved for a later slice), and `orchestrate` runs ONE bounded `buildRetrievalContext` read-only retrieval before the work call when the flag is ON, the directive is `'local'`, the brain did NOT already ground the turn (`brainGroundedness !== 'grounded'` — idempotent, no double-retrieval), and a `researchPort` is wired. Findings are carried into execution as a new `local-investigation` grounding block (threaded through the existing context-block seam → work-call + hedge + panel, rendered right before INTENT). Subscription-OAuth only: REUSES the existing `ResearchPort` + the existing bounded caps (`RETRIEVAL_MAX_HITS=6`, `RETRIEVAL_MAX_FILES=3`) + the no-improvement stop — NO new model call, network, embeddings, or quota meter. Fail-soft throughout (absent port / empty findings / abort all degrade cleanly). **Neutral by default**: flag OFF ⇒ the directive has no `requiredInvestigation` field, the preflight is dead, `investigationContext` stays '' and is omitted from the deps copy + prompt ⇒ the directive, deps copy, work prompt, and event stream are byte-identical to today. Full gate green: 6137 unit/arch pass (+21: OFF byte-identity / no retrieval call, ON exactly-one-call + findings reach the work prompt, idempotent-when-grounded, none-skips, no-port fail-soft, abort→cancel final, the derive truth-table, the flag truth-table, and an end-to-end menu flag-threading test), UI 128, all 4 PTY smokes, tsc/eslint/knip clean. Built via the opencode **Go** lane (`opencode-go/kimi-k2.7-code`); the conductor caught + reverted unrelated JSDoc churn the lane introduced in `prompt-context.ts` before commit (the rank-9 additions there are pristine). The last remaining audit item is P2 rank 10 (aggregate preflight-overhead guard).

## 3.139.0
fix(menu): the "Not signed in yet" empty-state hint now lists `[p] grok` alongside `[j] Claude · [k] Codex · [o] opencode`. grok was already a first-class menu citizen everywhere else (the `[p] Login grok` Auth entry, the provider-status display row when installed, and the first-run "Add grok?" welcome prompt) — this closes the one straggler where the onboarding hint omitted the working `[p]` key. Pure display string; full gate green (6116 unit pass, UI 128, menu PTY smokes), tsc/eslint clean.

## 3.138.0
fix(providers): reconcile the grok adapter against a LIVE grok transcript (G2) — the 3.137.0 provisional adapter was modeled on Claude's schema and would NOT have worked. Captured real `grok … --output-format streaming-json` output after `grok login --device-auth` and fixed four real bugs the provisional version shipped: (1) **Parser** — grok's streaming schema is `{"type":"thought","data"}` / `{"type":"text","data"}` / `{"type":"end","stopReason","sessionId","requestId"}` (NO usage/cost/tool events — cost is pricing-table-estimated like codex), nothing like Claude's `stream_event`/`assistant`/`result`. Rewrote `grok-parse.ts` from `parseGrokLine` (stateless, Claude-shaped) to `createGrokParser()` — a STATEFUL factory (codex/opencode pattern) that accumulates `text` fragments and emits the terminal `done` with the full text + grok `sessionId` on `end`, maps `thought`→`reasoning`, and stays tolerant of unknown/malformed lines. (2) **Invalid invocation** — `grok --single` requires an inline `<PROMPT>` value and CANNOT combine with `--prompt-file` (errors live); dropped `--single` so the adapter runs `grok --output-format streaming-json -m <model> … --prompt-file <PATH>`. (3) **Invalid permission mode** — `--permission-mode restrictive` does not exist (grok's set is default|acceptEdits|auto|dontAsk|bypassPermissions|plan); grok has a separate OS-level `--sandbox off|workspace|read-only|strict` guardrail, so `grokSandboxArgs` now pairs the real sandbox profile with a non-prompting permission mode (read-only→`--sandbox read-only --permission-mode dontAsk`, workspace-write→`--sandbox workspace --permission-mode acceptEdits`, full-access→`--sandbox off --permission-mode bypassPermissions`). (4) **Model parse** — real `grok models` prints `  - grok-build` / `  * grok-composer-2.5-fast (default)`; `parseGrokModels`' whole-line regex dropped the default model on its ` (default)` suffix, so it now takes the leading token per line. Also corrected the fictional `grok-4.3` model id to the real roster (`grok-build` ~1M-ctx coding agent; `grok-composer-2.5-fast` fast default) across pricing + model-capabilities, and retiered worker→`grok-composer-2.5-fast`. Verified the exact adapter argv end-to-end against live grok (clean thought/text/end, text accumulates correctly, sessionId captured). Full gate green: tsc + eslint + knip clean, **6116 unit/arch pass**, UI **128** (one Ink-timing flake cleared on re-run), all 4 PTY smokes. Subscription-OAuth only — auth unchanged (`grok login --oauth`/`--device-auth`, creds in `~/.grok/`).

## 3.137.0
feat(providers): add **grok** as a first-class provider (xAI's official "Grok Build" CLI, `@xai-official/grok`), alongside claude/codex/opencode — neutral-by-default. grok is a Claude-Code-style coding CLI, so the adapter (`src/providers/grok.ts`) closely mirrors `claude.ts`: headless run via `grok --single --prompt-file <PATH> --output-format streaming-json -m <MODEL>`, with `--effort` (identical low|medium|high|xhigh|max ladder), `--permission-mode` mapping (read-only→restrictive, workspace-write→acceptEdits, full-access→bypassPermissions), web-search opt-out (`--disable-web-search` unless requested), and `--resume`. **Subscription-OAuth ONLY** — the adapter NEVER sets `XAI_API_KEY`, NEVER uses an `sk-` key, NEVER passes `--xai-api-base-url`; auth is exclusively `grok login --oauth` / `grok login --device-auth` (device-code for headless/Replit), with creds living in grok's own `~/.grok/` store — myshell never sees the secret, exactly the opencode/codex model. Detection (`detectGrokProvider`) mirrors opencode: `grok --version` for install, `grok models` for auth+model-list (it exits 0 even unauthed, printing "You are not authenticated."). Wired end-to-end through the `ProviderId` union, registry (install-gated: only built when `env.grok.installed`), login command table (interactive + device-code), pricing (default `grok-build`), declarative model capabilities, policy `providerOrderByTier` (appended **LAST** on every tier), cli/doctor health, and the menu/welcome sign-in flow. **Neutral by default**: grok is install-gated AND ordered last, so behavior is byte-identical when grok isn't installed; an installed-but-unauthenticated grok is never routed to (route prefers authenticated+available first). The streaming-json parser (`grok-parse.ts`) is modeled on `claude-parse.ts` and marked PROVISIONAL pending a live-transcript reconciliation pass (G2) once an authenticated session can be captured. Full gate green: tsc + eslint + knip clean, lockfile firewall 0, **6118 unit/arch pass (+52)**, UI **128 pass** (fixed two `test/ui` env fixtures that omitted the now-required `EnvironmentStatus.grok` field — only caught by `test:ui`, since `tsc --noEmit` doesn't type-check the `.tsx` UI tests), all 4 PTY smokes green. Built via the opencode **Go** executor lane (`opencode-go/kimi-k2.7-code`) to spare Claude/GPT agent quota, then verified + gated locally before commit.

## 3.136.0
feat(quality): intent-derived risk signals — menu wiring (audit rank 8 S6 — **rank 8 complete**). The menu now resolves the rank-8 gate and threads it into orchestrate, a verbatim mirror of the rank-7 unify-preflight wiring: `const riskSignalsOn = preflightRiskSignalsEnabled(process.env, mutableCtx.config);` next to the `unifyPreflightOn` read, and `...(riskSignalsOn ? { riskSignals: true } : {})` in the orchestrate-deps spread inside `buildDeps`. Default OFF ⇒ the dep is omitted ⇒ orchestrate's OFF path (risk stays `det.risk`, hint fields stripped before engagement) runs byte-identical. Includes an end-to-end flag-threading test that drives `startMenu` with a deterministically-LOW task whose model intent frame carries `operationRisk:'critical'` and asserts: flag OFF ⇒ risk stays `low` (hints stripped), flag ON ⇒ `riskSignals: true` threads through and `combineRisk` raises the turn low→`critical` — proving the whole rank-8 chain (flag → deps → preflight → combineRisk → classification) end to end through the real menu. Full gate green: 6066 unit/arch pass (+2 flag-threading), UI 128, all 4 PTY smokes (smoke:pty passed first-chained, no flake), tsc + eslint + knip clean. Subscription-OAuth only — no new model call, cache, or network surface. Slice S6 — **audit rank 8 (intent-derived risk signals: the model may RAISE risk on genuine evidence, never lower the deterministic floor; + the additive externalFreshness→web-research term) is complete**, all behind the default-OFF `MYSHELL_RISK_SIGNALS` / `config.experimentalRiskSignals` gate. The next deferred audit items are P2 ranks 9–10.

## 3.135.0
feat(quality): intent-derived risk signals are WIRED — the model can now RAISE risk on genuine evidence, never lower it (audit rank 8 S5). Behind a default-OFF gate (`MYSHELL_RISK_SIGNALS` / `config.experimentalRiskSignals`), `orchestrate`'s preflight now combines the intent frame's optional `operationRisk`/`blastRadius` hints (3.134.0) with the deterministic keyword risk via `combineRisk` (3.133.0) on BOTH preflight branches, so a genuinely dangerous or wide-blast task the keyword classifier scored `low` gets the engagement safety floor / escalation / review it deserves — while the deterministic risk stays a hard FLOOR the model can never undercut. Two gated helpers keep the design honest: `raiseRisk(base, frame)` calls `combineRisk` ONLY when the flag is on and a frame exists (OFF or no frame → returns `base` unchanged, so `classification.risk` stays exactly `det.risk`), and `frameForDownstream(frame)` strips the three rank-8 hint fields from the frame copy that flows into `buildEngagementSignals` when the flag is OFF, so `needsExternal` sees `externalFreshness === undefined` (a dead branch) and the `WEB_RESEARCH` determination is byte-identical. On the ON path the frame passes through and the new ADDITIVE freshness term in `needsExternal` fires — `'required'` adds +2 (enough to reach `RESEARCH_T` alone), `'helpful'` adds +1 (strictly below the bar, can never trigger web search unaided), `'none'`/absent 0 — still through the SAME bias-shifted threshold and the same soft, fail-soft route pre-pass, so a calm local turn is never pushed into a web search. **Event-ordering decision** (documented inline): the unified branch yields `classified` AFTER extraction so it carries the raised risk; the else branch yields `classified` BEFORE extraction and that event was deliberately NOT moved (moving it would break the OFF-path event sequence) — the raised risk still reaches engagement + `taskSignals.risk` via re-raise after extraction. Fail-soft preserved (extractor throws/null → no frame → no raise/strip, exact today's fallback); abort and the brain-loop re-extraction (a2b) untouched. **Neutral by default**: with the flag OFF every line is inert on BOTH axes — `classification.risk` stays `det.risk`, the web determination is unchanged, and the existing orchestrate / oracle / engagement / menu-flow suites pass byte-identical. Full gate green: 6064 unit/arch pass (+9: OFF risk byte-identity, ON low→high / blastRadius / max-of-two raise, critical-floor-wins, fail-soft no-frame, ON `required`→web fires, ON `helpful`-alone does NOT, OFF `required` does NOT — proving the strip), UI 128, all 4 PTY smokes, eslint + knip clean. Subscription-OAuth only — consumes fields already on the single gated extraction + the existing web-research path; no new model call, cache, or network surface. Slice S5 of audit rank 8; only the menu wiring (S6) remains.

## 3.134.0
feat(quality): intent-derived risk signals — frame extension + prompt (audit rank 8 S3+S4), additive and byte-neutral. Extends the SINGLE gated intent extraction to optionally carry three new risk signals — no new model call, just extra optional fields on the frame the model already emits. (1) `IntentFrame` gains three OPTIONAL readonly fields: `externalFreshness?: 'none'|'helpful'|'required'`, `operationRisk?: Risk`, `blastRadius?: Risk` — `operationRisk`/`blastRadius` REUSE the existing `Risk` ladder so the rank-8 `combineRisk` (3.133.0) can compare them directly against the deterministic floor; `externalFreshness` is the freshness vocabulary that will (later, gated) feed the existing additive web-research gate. (2) Tolerant parse/cap exactly mirroring rank-7's `routeTier`/`routePlan`: `parseIntentFrame` forwards the raw values (never validates, never a parse failure), and `capIntentFrame` validates against `VALID_RISKS`/`VALID_FRESHNESS` and OMITS any invalid/unknown/absent value — so an uncertain or garbled model value is simply dropped to `undefined` (which `combineRisk` treats as no-effect). `rulesIntentFrame` is untouched, so every rules/skipped frame keeps the fields absent. (3) `buildIntentPrompt` describes the three optional fields (wording mirroring the deterministic risk tables' subjects — deletes/deploys/migrates/secrets for `operationRisk`, local→shared→cross-cutting→production for `blastRadius`, the `needsExternal` lexicon for `externalFreshness`) and extends the JSON exemplar, instructing the model to raise above `low`/emit `required` ONLY on clear evidence and to OMIT when unsure. Everything is additive: the fields are absent on every existing frame, the prompt additions are additive substrings (no golden moved), and NOTHING consumes the new fields yet (the live orchestrate/engagement wiring is S5, behind the default-OFF `riskSignals` gate). Full suite byte-identical+: 6055 pass (+5 new intent tests: valid values parse+survive cap, invalid→omitted/no-throw incl. a non-string, absent→structurally-identical frame, and the prompt contains the three field names + exemplar keys). One faithful deviation from the design: `as IntentFrame['externalFreshness']` does not compile under `exactOptionalPropertyTypes: true`, so the equivalent literal-union cast `as 'none'|'helpful'|'required'` is used (the exact analogue of rank-7's `as Tier`). Subscription-OAuth only — no new model call, cache, or network surface. Slices S3+S4 of audit rank 8; the live orchestrate/engagement wiring (S5) + menu wiring (S6) follow behind the default-OFF gate.

## 3.133.0
feat(quality): intent-derived risk signals — the foundation (audit rank 8 S1+S2), all additive and byte-neutral. The audit found risk is decided 100% deterministically by the keyword classifier (`classify`), so a genuinely dangerous or wide-blast-radius task phrased WITHOUT a trigger word silently scores `low` and bypasses the engagement safety floor / escalation / review that a correct risk would fire. Rank 8 is the risk-axis twin of rank 7: it will let the single gated intent extraction RAISE risk on genuine evidence while the deterministic keyword risk stays a hard FLOOR (the model can raise, never lower; absent/invalid → no effect). This slice ships the pure foundation, none of it wired yet: (1) `combineRisk(deterministicRisk, { operationRisk?, blastRadius? })` — a pure MONOTONIC combine that initializes at the deterministic floor and only ever `Math.max`-es the severity rank upward over the EXISTING `Risk` ladder (`low < medium < high < critical`), so `combineRisk(d, h) >= d` for all hints, with equality when both are absent or below the floor; structurally the risk-axis mirror of `combineRoute`'s tier-raise + risk-lock discipline; (2) a default-OFF `preflightRiskSignalsEnabled` flag (env `MYSHELL_RISK_SIGNALS` ∈ {1,true,on,yes} OR `config.experimentalRiskSignals === true`), a verbatim clone of rank-7's `preflightUnifyEnabled` (reuses the same `UNIFY_ON` set, never throws); plus the `experimentalRiskSignals?` config field and the `OrchestrateDeps.riskSignals?` dep, each a sibling to its rank-7 neighbor. To respect the no-orphan arch guard without dead live wiring, the pure logic + flag live in the already-imported `router.ts` (which already imports `Risk`) — the eventual orchestrate/menu wiring (S5/S6) will import `combineRisk`/`preflightRiskSignalsEnabled` from there. Everything is additive and default-off → the full suite is byte-identical (6050 pass, +12 new tests: monotonicity incl. "model cannot demote a critical task", absent→identical over all four floors, max-of-two-hints, the exhaustive monotone-up loop, and the flag truth table incl. a throwing-env-getter case; router + intent goldens unchanged; eslint + knip + arch-guard green). Subscription-OAuth only — no new model call, cache, or network surface. Slices S1–S2 of audit rank 8; the frame extension + prompt (S3/S4), the delicate live orchestrate/engagement wiring (S5), and the menu wiring (S6) follow behind the same default-OFF gate.

## 3.132.0
feat(quality): the unified preflight is wired — one model call now does the work of two on ambiguous substantial turns (audit rank 7 complete). Behind a default-OFF gate (`MYSHELL_UNIFY_PREFLIGHT` / `config.experimentalUnifyPreflight`), `orchestrate`'s preflight collapses the two serial worker-tier calls (router classifier + intent extractor) into ONE: when the intent extraction was already going to run (`unifiedPreflightApplies` — gate on, the intent pass is scheduled, an extractor is wired), the router model call is **suppressed** and the route decision is derived from the intent frame's optional `routeTier`/`routePlan` hints via `combineRoute` (3.131.0) — keeping `risk` 100% deterministic (a model hint can raise the firepower tier, never lower the security floor). The §A.4 routePlan-ordering subtlety is handled by computing the extraction gate with the conservative `routePlan: false` (the router's plan flag isn't available pre-extraction; `false` can only ever skip, never add a call), then letting the frame's `routePlan` hint drive the downstream value. **Pure consolidation, provably never an addition**: the unified path runs ONLY when the intent call was already scheduled and REPLACES {router + intent} with {intent} — a mandatory call-count-parity test asserts gate-ON ambiguous-substantial turns make exactly ONE preflight model call (router 0 + intent 1) vs today's two (router 1 + intent 1), and that no turn class increases the call count (trivial turns stay 0/0; evidence turns stay 1 router; no-extractor turns fall back to `decideRoute`). Fail-soft preserved (extractor throws/null → `combineRoute` with no hints → the exact deterministic `decideRoute` fallback, no router retry); the brain-loop re-extraction (a2b), abort, and signal handling are untouched (only the first preflight extraction is unified). **Neutral by default**: gate OFF ⇒ the verbatim `else` branch (today's `decideRoute` + intent block, moved unchanged) runs ⇒ orchestrate, router, intent, and menu-flow characterization + Oracle goldens are byte-identical (full gate green: 6038 unit/arch pass, +9 new tests incl. the call-count parity matrix and end-to-end flag-threading through `startMenu`; UI 128, all 4 PTY smokes, knip clean). Subscription-OAuth only — reuses the existing intent extractor provider port + JSON frame, no new model-call type, cache, or network surface. Slices S4 (orchestrate branch) + S5 (menu wiring) — **audit rank 7 (unify route + intent preflight) is complete.** The next deferred audit items are P2 ranks 8–10.

## 3.131.0
feat(quality): the preflight-unification foundation — pure pieces to let one model call do the work of two, all additive and byte-neutral (audit rank 7). The audit found that on an ambiguous, substantial turn (no keyword tier evidence + multi-clause/long, both default-on engines) the preflight fires TWO serial worker-tier model calls — the router classifier AND the intent extractor — when the intent frame could simply carry the routing hints. This slice ships the foundation to collapse them to one, none of it wired yet: (1) `combineRoute(det, hints)` — a pure monotonic combine that takes the model's optional `routeTier`/`routePlan` hints but keeps `risk` **100% deterministic** (`det.risk` only; there is no `routeRisk` field anywhere, so a model hint can raise the firepower tier but can never lower the security/irreversibility floor), structurally mirroring `decideRoute`'s existing model branch; (2) `unifiedPreflightApplies(input)` — the pure predicate (`gateOn && runIntentScheduled && hasExtractor`) that will select the unified path ONLY when the intent call was already going to run, so it can ever only *remove* the router call, never add one; (3) optional `routeTier?`/`routePlan?` on `IntentFrame` with tolerant parse (invalid → omitted, never a parse failure), cap passthrough, and two extra optional fields in the intent prompt mirroring the router's tier definitions — reshaping the content of the single existing extraction, no new call; (4) a default-OFF `preflightUnifyEnabled` flag (`MYSHELL_UNIFY_PREFLIGHT` / `config.experimentalUnifyPreflight`). To respect the no-orphan arch guard without dead live wiring (a brand-new module would have no `src` importer until the deferred orchestrate/menu wiring lands), the pure logic + flag live in the already-imported `router.ts` rather than new orphan files; the eventual wiring (S4/S5) will import these names from there. Everything is additive and default-off → the full suite is byte-identical (6029 pass, +26 new tests: the monotonic combine incl. a "model hint cannot demote a critical task" assertion, the predicate truth table, the flag truth table, the frame parse/cap cases; router + intent goldens unchanged; knip + arch-guard green). Subscription-OAuth only — no new model call, cache, or network surface. Slices S1–S3 of audit rank 7; the delicate live `orchestrate` preflight branch (S4) + menu wiring (S5) follow behind the same default-OFF gate.

## 3.130.0
feat(quality): bounded cross-goal concurrency is wired — Phase D complete. Behind the EXISTING default-OFF `schedulerEnabled` gate, the goal launcher now runs the decomposed goals through `runSchedule` under a hard, right-sized cap instead of an uncapped pressure/provider limit. After `decompose` returns its goals, the launcher computes a four-way cap from real signals: the tuning ceiling (`concurrencyCeilingForRegime(regimeForIntensity(resolvedIntensity))` — focused/pair → 1, fleet* → 2, sourced the same way the planner resolves intensity, `resolveIntensity`/`autoIntensityForTurn`), the governor call-budget ceiling (the mode-minus-pressure headroom collapsed to 1-vs-≥2, mirroring `planningSelectionEntitlement`), and the **genuine parallel demand** (the count of decomposed goals with no `dependsOn` — the DAG roots that can start immediately). Their `min` is passed as the new additive `ScheduleDeps.maxActive`, which `runSchedule`'s live `currentActiveLimit()` forwards into `planSchedule`'s existing D3 clamp; combined with the in-scheduler pressure/provider `activeLimit` this yields exactly `crossGoalCap = min(activeLimit, tuningCeiling, callBudgetCeiling, genuineParallelGoalCount)`. **The no-overkill / birdhouse guarantee holds end-to-end:** a single-goal (or all-dependent, one-root) decomposition has demand 1 ⇒ cap 1 ⇒ one goal at a time, byte-identical to today's scheduler path; and raising tuning alone never adds a concurrent goal because the four-way `min` is blocked by the demand and budget terms (`BASE_ACTIVE_LIMIT=2` stays the absolute ceiling — the cap only ever *lowers* the limit). **Neutral by default**: `schedulerEnabled` is OFF, so the whole block is skipped (byte-identical to 3.129.0), and `ScheduleDeps.maxActive` absent is byte-identical in the scheduler. No verdict writes, no new store method/cache/network surface; ESC/abort fan-out (`schedAc.signal` → child controllers) is untouched. Full gate green: 6003 unit/arch pass (+ a menu-flow proof that scheduler-ON + a single-goal decomposition runs exactly one goal/one phase, and scheduler tests that `maxActive` lowers the live limit while absent keeps today's numbers), UI 128, all 4 PTY smokes, knip clean. Subscription-OAuth only — concurrent goals reuse `decompose`/`runSchedule`/`orchestrate` on authenticated providers. Slice D6 — **Phase D (concurrent execution: per-item park-and-continue + bounded cross-goal concurrency) is complete.**

## 3.129.0
feat(quality): per-item park-and-continue is wired — a blocker on one roadmap item no longer stalls the whole goal. Building on the D-foundation (3.128.0), the manager cycle now, behind the default-OFF `itemParkingEnabled` gate (`MYSHELL_ITEM_PARK` / `experimentalItemParking`), responds to a fork on an item (`turn.final.success === true && turn.final.questions !== undefined`) by **parking that one item and continuing on its unblocked siblings** instead of stopping the cycle. The parked item is set `status: 'blocked'` via the existing `setRoadmapItemStatus`, its open question is recorded in the item text behind the same `CLARIFY_PREFIX` marker that `itemBlockReason` classifies as `'clarify'` (reusing the existing `updateRoadmapItem` text patch — no new store method or write path), the live roadmap is refreshed so `pickNextTodo` skips it, and the cycle `continue`s to the next sibling. Both store writes are fail-soft (a store miss never breaks the cycle), the parked item gets **no verdict** and is never marked done, and when every item ends blocked the cycle breaks with the honest open-goal stop receipt (`pickNextTodo` → null, named blocker, "N/M to-dos verified. Keeping the goal open.", goal stays `running`) — never a silent done. **Neutral by default**: with the flag OFF (the shipped default) the fork takes exactly today's path — surface the fork, run the selector, stop the cycle — byte-identical to 3.128.0. Full gate green: 6000 unit/arch pass (+2 new ON-path menu-flow tests asserting the fork parks the item with `CLARIFY_PREFIX`/`itemBlockReason === 'clarify'`, the cycle advances to the sibling, and the all-blocked open-goal receipt fires; plus the OFF neutrality assertion), UI 128, all 4 PTY smokes, knip clean. Subscription-OAuth only — no new model call, cache, or network surface. Slice D5 of Phase D; the cross-goal cap wiring (D6) follows behind its own default-OFF `schedulerEnabled` gate.

## 3.128.0
feat(quality): the concurrent-execution foundation — pure decisions + dark switches for Phase D, all additive and byte-neutral. Investigation confirmed Phase D is overwhelmingly a *wiring* phase: the cross-goal scheduler (`runSchedule`/`planSchedule`) and the per-item readiness substrate (`pickReadyTodos`) already exist and are tested, just deliberately restricted to one goal / unwired. This slice adds the small pieces the wiring will need, none of them live yet: (1) `itemBlockReason(item, byId)` — a pure classifier returning `'dependency' | 'clarify' | 'unverifiable' | null` from existing fields (unmet `dependsOn` dominates, then a `Clarify:` text marker, else an exhausted self-heal), centralizing "why is this item parked" for honest surfacing; (2) `concurrencyCeilingForRegime` (focused/pair → 1, fleet* → 2, never above `BASE_ACTIVE_LIMIT`) + `crossGoalCap` — a load-bearing `min` of provider/pressure limit, tuning ceiling, call-budget ceiling, and *genuine independent-goal demand*, so turning the tuning dial up alone never adds a concurrent goal (a one-goal or sequential plan caps at 1 even at max tuning, the birdhouse guarantee); (3) an additive optional `maxActive` clamp on `planSchedule` that can only *lower* the active limit and is byte-identical to today when absent; (4) a new default-OFF `itemParkingEnabled` flag (`MYSHELL_ITEM_PARK` / `experimentalItemParking`) so the upcoming per-item park-and-continue change (which touches the default-on manager cycle) ships dark. Everything is pure/additive in already-imported modules; nothing is wired, so the full suite stays byte-identical (5998 pass, knip clean, arch no-orphan green). ~30 new truth-table tests pin every branch incl. the `maxActive`-absent-=-today neutrality and the birdhouse-at-max-tuning cap. Slice D-foundation (D1–D4) of Phase D; the item-parking wiring (D5) and the cross-goal cap wiring (D6) — both behind their default-OFF gates — follow. Subscription-OAuth only; no new store method, cache, or network surface.

## 3.127.0
feat(quality): multi-brain plan selection is wired — Phase C complete. Behind the same default-off gate as Phase B (`MYSHELL_PLANNING_DEPTH` / `experimentalPlanningDepth`), the preflight goal planner now activates the full Phase C machinery for a genuinely hard goal whose first plan is genuinely deficient, at high tuning with the budget and providers to afford it. When the selection entitlement is unlocked (`planningSelectionEntitlement`: gate on, intensity 4–5, call budget ≥3, governor `panelAllowedForShape` true, ≥2 authenticated providers) AND the trigger fires (`shouldRunPlanningSelection`: a plan-fixable deficiency in candidate A — missing approach/`DONE:`, dropped caps, or generic-only structure — on a hard/high-stakes goal, with all the absolute exclusions cleared), the planner runs candidate A via the new attempt-exposing variant (`makeGoalPlannerAttempt`, which surfaces A's provider/model/raw while keeping `makeGoalPlanner` byte-identical), then calls the C3 engine (`selectGoalPlan`) to get one different-provider candidate B and one adjudicator, prints an honest up-front notice and terminal disclosure, and feeds the single selected `GoalPlan` into the unchanged normalization → confidence → activation → verified-done path. The planner tier is now chosen by `choosePlannerTier` (IC baseline, manager only on genuine need within the tuning ceiling). **Budget discipline is exact**: when selection is unlocked the cold-cache L2 grounding await is *skipped* so the worst case stays A + B + adjudicator = 3 calls — never a 4th. When the entitlement is locked (the common case, any simple/birdhouse goal even at max tuning, single-provider, cost-saver, or high pressure) the path is exactly Phase B, and gate-off is byte-identical to 3.126. `assessGoalConfidence`, `decideGoalActivation`, the post-turn auto-stage path, and `runPanel` are all untouched; `panelAllowedForShape` is now an additive export. Full gate green: 5957 unit/arch pass, UI 128, all 4 PTY smokes, knip clean; new menu-flow tests assert the selection path, the exact 3-call budget (filtering planner/adjudicator traffic so it can't be masked), and every lock/veto. Subscription-OAuth only — candidate B and the adjudicator run through the existing provider machinery, no API/SDK/network/billing surface. Slice C4 — **Phase C (multi-brain plan selection + tuning-gated model choice) is complete.**

## 3.126.0
feat(quality): the multi-brain plan-selection engine — get a second plan from a different subscription provider and adjudicate, reusing the panel machinery without touching it. This is the bounded engine Phase C's trigger (C1) will gate; it is **additive and not yet wired** (the menu seam is C4). `selectGoalPlan` takes an already-run first plan (candidate A) and, via an injectable runner that defaults to the existing `runCandidate`/`mergeCandidates` executor, runs exactly ONE different-provider candidate B on the same planner prompt + grounding (8s cap), then ONE adjudicator that compares P1=A vs P2=B and must answer in the existing tagged `GoalPlan` grammar plus a final-line `{"choice":"P1"|"P2",...}` envelope (parsed by the shared `parseFinalLineChoiceEnvelope`/`tallyChoiceEnvelopes` — one adjudicator is disclosed as one adjudicator, never "consensus"). A pure `isAcceptableGoalPlanSynthesis` predicate accepts the synthesized plan only when it parses as `stage`, has a titled goal with todos, introduces no `clarify`, stays within parser caps, and is **non-regressive** vs the chosen candidate on `doneWhen`/approach/cap-drops — otherwise it returns the named original candidate, and if everything fails it falls soft to candidate A. Hard bounds: ≤2 plan candidates (A reused + 1 B), ≤1 adjudicator, ≤3 total calls, no retry/loop/rebuttal. It returns one `GoalPlan` + a `PanelDebateReceipt`-style receipt with honest up-front-notice and terminal-disclosure formatters (participants + real call count, never dollars/percentages/"free"). Built **entirely additively in `ensemble.ts` — `runPanel` and every panel/ensemble/characterization golden are byte-identical** (full suite 5949 pass); the pure prompt/acceptance/selection/notice pieces and the full failure matrix (B fails / empty / unparseable, adjudicator fails, P1 vs P2, invalid synthesis → named original, invalid envelope → A, clarify rejected, doneWhen regression rejected, exact call order, disclosure wording) are unit-tested with a faked runner. Subscription-OAuth only — candidates run through the existing `Provider.run` path, no API/SDK/network/billing surface. Slice C3 of Phase C; the menu seam + 3-call reservation (C4) wires it behind the default-off gate next.

## 3.125.0
feat(quality): the tuning-clamped planner-tier chooser — a stronger planning model only when genuinely needed *and* the tuning allows it. Three pure functions complete Phase C's decision foundation. `plannerTierCeiling` maps resolved intensity to the strongest reachable planner tier (1–3 → `ic`, 4–5 → `manager`). `needStrongPlanner` (tuning-independent) asks only whether the *task* warrants the stronger tier — high/critical risk, a risky/investigate shape, or a substantial `decide` turn whose first plan has a plan-fixable deficiency. `choosePlannerTier` combines them: it returns `manager` only when need asks for it AND the intensity ceiling permits it, otherwise `ic` — so low tuning never raises the tier even when the task wants it (the efficiency floor), and max tuning never raises it without genuine need (no overkill). This mirrors `route.ts`'s clamp pattern (difficulty moves at most one step, clamped to the ceiling, Efficient never raised); the final provider/model and policy `maxTier` clamp still happen at the route layer when this is wired (C4). Pure, in the already-imported `autonomy.ts`, nothing wired → fully neutral, every golden byte-identical (full suite 5937 pass). New truth-table tests pin the ceiling, the need predicate, and both clamp directions ("low tuning never raises even when needed", "max tuning does not raise without need"). Slice C2 of Phase C (after C1's selection trigger, 3.124.0); the ensemble candidate/selection adapter (C3) and the menu seam + 3-call reservation (C4) follow. Subscription-OAuth only — this only chooses an existing subscription tier, never a new service.

## 3.124.0
feat(quality): the multi-brain plan-selection gate — pure trigger functions that decide *whether* a second planning brain may ever run, with the anti-overkill guarantee built in. Phase C lets the partner, for a genuinely hard goal whose first plan is genuinely deficient, get a second plan from a different subscription provider and pick the best — but only when it's truly warranted. This slice ships the two pure decisions that gate it (no wiring yet). `planningSelectionEntitlement` is the CEILING: selection is `'unlocked'` only when ALL hold — the rollout gate is on, resolved intensity is 4–5, the governor call budget is ≥3, the governor's panel-shape allowance is true (cross-vendor, non-cost-saver, shape decide/risky/investigate), and ≥2 authenticated providers exist; any one failing → `'locked'`. `shouldRunPlanningSelection` is the NEED (zero tuning input): it returns false on absolute exclusions first (quick/explain, the low-risk reversible non-substantial *birdhouse*, a `clarify` or `none` first verdict, or a gap that's only `no-verification`), then requires BOTH a plan-fixable deficiency (no parseable plan, a substantial goal missing its approach, a non-trivial goal missing `DONE:`, dropped caps, generic-only fallback, or a not-confident/no-done-when result) AND genuine hardness (risky/investigate shape, substantial decide, high/critical risk, repo+substantial, or deep engagement). So a second brain and a pricier model are *exceptional responses to demonstrated insufficiency on hard work* — never a reward for turning the dial up: at intensity 5 with budget 3 and three providers, a birdhouse still returns false (two independent predicates block it). Both pure, in the already-imported `autonomy.ts`, nothing wired → fully neutral, every golden byte-identical (full suite 5915 pass). 30+ new truth-table tests pin the entitlement conjunction, every exclusion, deficiency-without-hardness, hardness-without-deficiency, and the birdhouse-at-max-tuning guarantee. Slice C1 of Phase C; the planner-attempt receipt + tuning-clamped tier chooser (C2), the ensemble candidate/selection adapter (C3), and the menu wiring (C4) follow. Subscription-OAuth only.

## 3.123.0
feat(quality): a hard goal at high tuning now grounds itself before planning — effort-governed planning depth, wired (default-off). Building on the B1 decision core (3.122.0), the preflight goal planner now, behind an internal opt-in gate (`MYSHELL_PLANNING_DEPTH` / `config.experimentalPlanningDepth`, default OFF), computes the planning-depth cap and right-sizes the actual depth from cheap preflight signals (risk/tier from `classify`, a repo-present proxy, resolved intensity via `resolveIntensity`/`autoIntensityForTurn`, and a mode-minus-pressure call-budget ceiling). When — and only when — the right-sizer selects **L2** (a genuinely hard goal whose tuning ceiling unlocks it) **and** the whole-picture `SystemModel` cache is cold, it prints one honest up-front notice ("Planning deeper: grounding this first") and **awaits one understanding pass with a tight 8-second timeout** (never the 120s background default), caches the result, and runs the existing planner **grounded** by it — falling soft to today's ungrounded single call on any failure/timeout (with an honest "grounding unavailable" note). Every other turn — the common case, a simple/birdhouse goal even at max tuning, a warm cache, cost-saver, or high context pressure — takes exactly today's path: one planner call, no extra pass, no notice. The confidence gate and activation policy are untouched; depth ends before the gate. **Fully neutral by default**: the gate is off, so `judgeGoal` runs its current body verbatim — characterization, Oracle, menu-flow, goal-plan, understanding, and post-turn auto-stage goldens are byte-identical (full suite 5898 pass, UI 128, all 4 PTY smokes, knip clean). Conservative on purpose: the cheap scope proxy can only *under*-trigger L2 (less depth, never overkill), and the post-turn auto-stage path is intentionally left at L1 for a later slice. Subscription-OAuth only — reuses the existing understanding generator and `systemModelCache`, no new cache, no API/network surface. Slice B2 of Phase B; the L3 refinement round remains deferred.

## 3.122.0
feat(quality): the planning-depth decision core — "tuning is a ceiling, never a floor," in two pure functions. Phase B makes how much investigation/planning the partner does before the confidence gate scale with the resolved tuning, but as a CEILING that *unlocks* depth for genuinely hard goals — never a floor that forces it on a simple one ("don't bring the excavator to build a birdhouse"). `planningDepthCap` computes the maximum reachable depth (L1 = today's single planner call; L2 = one grounded pass) as the MINIMUM of three independent ceilings — resolved intensity (1–2 → L1, 3–5 → L2), the governor call-budget ceiling (1 → L1, 2–3 → L2, so cost-saver and context pressure automatically cap it), and shape (`quick`/`explain` are absolute L1). The minimum is load-bearing: no high signal can cancel a lower quota. `chooseInitialPlanningDepth` then picks the SHALLOWEST sufficient depth from real scope alone — `repoOriented && substantial`, an `investigate`/`risky` shape, deep engagement, or high/critical risk earns L2; everything else stays L1 — and **tuning never appears in this selection** (it lives only in the cap), so a high-cap simple task still chooses L1. Both pure (no I/O), in the already-imported `autonomy.ts`, and **nothing is wired yet** → fully neutral, every characterization/Oracle/menu/governor golden byte-identical (full suite 5877 pass). 30+ new truth-table tests pin the cap math and the birdhouse guarantee (a high-cap reversible task chooses L1 at every intensity). Slice B1 of Phase B; the menu wiring (await one grounded understanding pass on a cold cache for a hard goal when the cap allows, behind a default-off gate) lands as B2, and the L3 refinement round is deferred until L2 proves out.

## 3.121.0
feat(quality): you can now tell the partner — in plain language — how to activate confident goals, per conversation. The lifecycle's default is adaptive (trivial/reversible auto-runs; substantial/risky relays its plan, 3.120.0). Now a standing instruction in chat overrides that for the current conversation: **"from now on just go when you're confident"** → confident goals auto-run (skip the await); **"always relay the plan first" / "run it by me first"** → every confident goal awaits your green light, even trivial ones; **"back to adaptive" / "use your best judgment"** → clears it. Detection is a **conservative pure** function (`detectActivationOverride`): it fires only on clear standing-policy phrasing (an explicit "from now on/always/by default" framing plus the intent, and "confident" for the go-variant) and returns null for ordinary work turns, questions, and one-off sequencing like "let's plan the migration first" — so it never hijacks a work request. The preference is stored **per-conversation**, mirroring the existing `intensity` override exactly: a new optional `ConversationMeta.activation` field + `setActivation` store method that canonicalizes adaptive to absence and is preserved across every unrelated metadata mutation (rename/pin/category/recap/intensity/**truncate** — the truncate path now also preserves both `activation` and `intensity`, fixing a latent drop of the intensity override on /retry and /edit). It takes effect the **same turn** it's set (detected and persisted before the goal turn runs) and only shifts *when* we auto-run vs relay — it never lowers confidence, verification, standing-rule, or verified-done requirements ('go-when-confident' still requires the goal to be CONFIDENT first). **Neutral by default**: no phrase + nothing stored → `setActivation` is never called, the override resolves to 'adaptive', and behavior + persisted JSON are byte-for-byte 3.120.0 (the `activation` field is omitted when absent). Subscription-OAuth only — detection is pure string logic, no model call, no network. Full gate green: 5828 unit/arch pass, UI pass, all 4 PTY smokes, knip clean. Slice A4 — **Phase A (the goal-lifecycle spine) is complete.**

## 3.120.0
feat(quality): auto-goal stops being "hear work → immediately run it" — the lifecycle spine lands. The pre-turn auto-goal path used to flip every staged goal straight to `running` and launch it. Now, on a staged plan, it creates the goal **parked** (inactive), runs the **confidence gate** (`assessGoalConfidence`, 3.118.0) over real evidence — the planner staged it, the goal is named, no genuine fork remains (forks already route to the clarify question upstream), a verifiable `DONE:` is present (3.119.0), and a real test command is detectable in the cwd (one cached `detectTestCommand` probe per cwd, fail-soft) — and only then applies the **adaptive-activation policy** (`decideGoalActivation`): a confident, trivial, reversible goal (≤2 steps, not high-stakes) **auto-runs** exactly as before (`On it —` → `runGoalLoop` → verified-done), while a **substantial** goal (>1 goal or ≥3 steps), a **high-stakes** one (the risk classifier flags e.g. billing/auth), or one that's **confident but unverifiable / lacks a done-criterion** stays **parked** and the partner **relays the plan** (`Staged — <title>` + roadmap + "say go / run /goals go to start") or reports the gap honestly (`Staged (holding) — … no test command detected to verify completion`). Verifiability is treated as **confidence evidence, not a dumb guard** — a non-verifiable goal is kept staged for review, never deleted and never turned into a fake preference question. Activation is driven **only by real scope signals** (shape / substantial / stakes / fork) — **tuning/subscription tier never appears**, so a high tier can't force the excavator onto a birdhouse. A new `'staged-parked'` return sentinel makes the parked paths fall through to normal chat **without** the post-turn auto-stage re-creating the same goal (no double-staging), and the launch caller + auto-engage path only ever launch a real goal object. Neutral where it must be: the planner-`none`, no-work-intent, clarify-before-create, auto-goal-**off**, and `/goals go` promotion goldens all stay byte-identical; only the "running before first worker" timing test was updated to a confident-trivial fixture (it still asserts auto-run), plus two new tests pin the substantial→parked and unverifiable→parked paths. Full gate green: 5819 unit/arch pass, 128 UI pass, all 4 PTY smokes (doubled-paste / ink / menu-ink / handoff) pass, knip clean. Subscription-OAuth only — no new model calls, no network. Slice A3 of Phase A (per-conversation activation override is A4).

## 3.119.0
feat(quality): the goal planner now states a **verifiable definition of done** per goal — the confidence gate's missing evidence. A staged goal already carried a professional title, an optional best-approach, and ordered todos; it now also carries an optional concise `DONE:` line — "how you KNOW the goal is finished" (a passing test, an observable behavior). The planner prompt asks for one short, checkable success criterion per goal (and to skip it only when truly trivial); the tagged-line parser captures `DONE:` into `GoalPlan.goals[].doneWhen` exactly like the existing APPROACH/WHY/ALT accumulation — first non-empty wins, capped at 160 chars, suppressed for a cap-dropped goal so it never bleeds onto a surviving one. **Purely additive / neutral by default**: a planner reply with no DONE line yields a goal with no `doneWhen` field at all (byte-identical to today) — the field is emitted only when present and non-empty. No new model call (the planner already makes one; DONE is one more tagged line it may emit) and no API/network surface — subscription-OAuth only. Honors the ceiling principle: DONE is one short line, never an essay. Full suite 5817 pass (the planner-prompt text isn't pinned by any golden); 6 new parser tests cover present / omitted-no-field / capped / dropped-goal isolation / first-wins / prompt-contains-DONE. Slice A2 of Phase A — feeds A3's confidence gate (`assessGoalConfidence`'s `hasDoneWhen` conjunct, 3.118.0).

## 3.118.0
feat(quality): the goal-lifecycle spine begins — two pure decision functions that make auto-goal behave like an elite pro running a deliverable, not "hear work → immediately run it." `assessGoalConfidence` is the **confidence gate**: a goal is only confident when the planner staged a real work turn, the intended outcome is non-empty, **no genuine owner-only fork remains**, an explicit done-when is present, **and** a real verification route exists. A lingering fork returns `needs-clarification` (ask one sharp question, never auto-run a real decision); a missing done-when or verification returns `not-confident` and **parks** the goal — it never fabricates a preference question out of a verification gap (folding in the stashed rank-6 verifiability requirement as one conjunct of confidence rather than a separate guard). Brain-dead-simple work — clear goal, no fork, obvious done-when, real verification — passes straight through with no question. `decideGoalActivation` is the **adaptive-activation policy**: not-confident → `hold`; otherwise the per-conversation override decides (`always-plan-first` → await green light even for trivial work; `go-when-confident` → auto-run), and in `adaptive` mode simple reversible `quick`/`build`/`explain` work auto-runs while substantial, high-stakes, forked, or investigative work awaits approval. Honors **"tuning is a ceiling, never a floor"**: activation is driven only by real scope signals (shape / substantial / stakes / fork) — no tuning or subscription-tier input appears, so a high tier can never force the excavator onto a birdhouse. Both functions are pure (no I/O, no new cross-module imports) and live in the already-imported `autonomy.ts`. **Fully neutral**: nothing wires them yet, so every characterization, Oracle, governor, and menu-flow golden is byte-for-byte unchanged (full suite 5812 pass). 21 new truth-table tests pin every branch. Slice A1 of Phase A (the menu wiring + per-conversation override land as follow-on slices A2/A3).

## 3.117.0
feat(efficiency): under heavy context pressure, history yields *first* — and the "older turns" notice now tells the truth. Completing priority-aware compaction (3.114.0): when a turn's raw assembled context wants more than its 6000-char cap, the conversation history budget is now reduced by exactly that overflow (`historyMaxChars = max(0, 6000 − (rawContext − 6000))`, oldest turns dropped first) so the lower-value blocks shed before the model's working context is squeezed — and if the overflow is large enough to zero the budget, history is omitted cleanly rather than fed a malformed single-truncated turn. `compactHistory` stays the single, once-called compactor shared by the panel and sequential paths (no double-compaction), and now handles non-positive and marker-sized budgets correctly. Crucially, the dim "N older turns are outside the model's context window" notice in the chat scrollback was previously computed from the *default* 6000 bound — so it could disagree with what the model actually received once the budget was reduced. It now fires from an observation callback carrying the turn's **real** effective bounds (and is deduplicated, and suppressed on native-session turns where replay doesn't apply), so the honesty note can never contradict the actual replay. Fully neutral on the common path: when raw context is ≤6000 (every existing fixture and the vast majority of turns), history uses the exact same `compactHistory(replayHistory)` call and bytes as before — characterization + Oracle goldens unchanged, and the observation seam is inert when no callback is supplied. Audit P1 #5 slice S2 — **P1 #5 complete**.

## 3.116.0
feat(efficiency): large governed panels now synthesize from compact candidate summaries instead of full transcripts. When a governed panel's two candidates produce a lot of text, the synthesizer prompt embedded **every full candidate answer** — burning tokens on the adjudication call. Now, only on the governed-panel path (`governorPlan` present) and only when the candidates' combined output exceeds **12,000 characters**, each candidate is rendered as a compact record: its parseable final-line **conclusion envelope whole**, plus a bounded **answer excerpt** (first 1,600 + last 400 chars, with a `…[candidate body compacted]…` marker; bodies ≤2,000 chars are kept whole). The tail is deliberately retained because conclusions, caveats, and test/failure notes cluster there. The compaction is an explicit opt-in argument to `buildPanelSynthesisPrompt` and reuses the existing `splitFinalLineJson` seam — no new parser. Strictly neutral otherwise: governor-off panels, governed panels under the 12k threshold, and the disagreement critique-synthesis path (`buildPanelCritiqueSynthesisPrompt`, intentionally left full for now) are all byte-for-byte unchanged — the exact synthesis golden and the governed consensus/unknown transcripts stay identical, as do the characterization + Oracle goldens. The 12k threshold governs the candidate-answers prompt section only (unrelated to the 6000 context-block cap). Audit P1 #5 slice S3.

## 3.115.0
fix(efficiency): routing now estimates the context the model *actually* receives, not the raw producers. The token estimate that feeds task-kind derivation and large-context capability routing was summing the **raw** context producer strings and **omitting** taste + system-understanding — so it could believe a turn carried far more context than the 6000-char assembly cap actually emits (an oversized environment/repo-map block inflating the estimate) or less than it really sends (taste/understanding silently uncounted). Now the estimate is built from the **rendered** assembled context: a single shared `ContextBlockOptions` builder (`context-block-options.ts`) produces the exact option set the initial executor's prompt uses, and the estimate measures `[task, history, assembleContextBlocksDetailed(...).text]` — the same bytes the executor sends, with no double-counting of individual fields. The shared builder also removes the drift risk between estimation and prompt construction (they can no longer diverge by a hand-maintained field list); panel synthesis deliberately keeps its current behavior (`includeUnderstanding: false`) so its prompt bytes are unchanged. Pure estimate/routing-input correction: **no prompt bytes move** at any call site, and the only routing effect is when a corrected estimate genuinely crosses a large-context threshold. Characterization + Oracle goldens unchanged; `estimateInputTokens` (chars/4) untouched. Audit P1 #5 slice S4.

## 3.114.0
fix(quality): a standing rule can no longer be silently dropped when context is tight — priority-aware compaction. The per-turn context blocks (environment, tool-state, memory, taste, work-state, goals, **standing rules**, intent, …) were assembled in a fixed order and, if the total exceeded the 6000-char backstop, simply **tail-sliced** at 6000 — which is priority-*inverted*: it cut the LAST blocks first (standing rules, intent, engagement, system-understanding) and could even slice mid-rule, so a user-authored NEVER/PAUSE/PREFER policy could vanish from the prompt under cap pressure. Now compaction is **priority-aware and whole-block**: blocks carry a tier — **non-sheddable** (work-state, salvaged-draft, goals, standing rules, intent) are never removed or sliced; **shed-first** (environment → tool-state → learned-taste) are dropped first; then **degradable** (partner-nudge → engagement → vision-triage → system-understanding → memory) — removing whole blocks in that order, rejoining and stopping the moment it fits, always preserving canonical block order among survivors. Nothing is ever character-sliced. If the non-sheddable blocks *alone* exceed the cap, they're returned whole (an explicit, observable safety overflow surfaced via a new additive `assembleContextBlocksDetailed` → `{ text, rawLength, overflowedNonSheddable }`) rather than ever truncating a rule — the audit's required guarantee. Fully neutral on the common path: when assembled context is ≤6000 chars (every existing fixture, and the vast majority of real turns) the function returns the exact same string as before — byte-for-byte — so the characterization and Oracle goldens are unchanged; only the previously-crude over-cap path changes. The 6000 hard cap stays. First slice of priority-aware context compaction (audit P1 #5); history-pressure composition, governed large-panel candidate summaries, and the rendered-input token-estimate fix land as follow-on slices.

## 3.113.0
feat(quality): when two panel brains genuinely disagree, the final call now critiques the split instead of blind-synthesizing — within the same 3-call budget. Building on 3.112.0's shape-aware admission, a governed panel (`decide`/`risky`/`investigate`, ≥2 vendors, budget 3) now asks each of its two independent candidates to end with a structured conclusion envelope, then **deterministically** classifies their agreement using the *same* honest final-line parser and tally the judgment poll uses (extracted into a shared `judgment-shared.ts`): `consensus` (same option), `material-disagreement` (two valid, different option IDs), or `unknown` (anything unparseable). On **material disagreement only**, the third-and-final adjudication call becomes a *combined critique-and-synthesis*: the synthesizer sees both candidates' conclusions and answers, explicitly critiques the split, reconsiders, and writes one final answer — which still flows through the shared verify→repair→accept gate (3.108) like any other. The honest budget boundary is respected exactly: a true multi-round debate (2 candidates + synthesis + critique + re-synthesis = 5 calls) is **impossible** under the hard 3-call cap and is never faked — material disagreement is detected *before* synthesis so the existing third call carries the critique (no extra call), and low *synthesis* confidence (only knowable *after* synthesis) is recorded as an honest "trigger observed, not run (budget exhausted)" notice with **no** fourth call. Receipts are additive `notice` events (`ran (material disagreement)` / `not run (consensus)` / `not run (no parseable decision split)` / the budget-exhausted line), governor-on only, never claiming "ran" until the critique call actually starts. Strictly conservative: `OPEN`/no-fork-vocabulary can never manufacture disagreement, consensus is an absolute veto (`agreement === 'material-disagreement'`, never `!== 'consensus'`), candidate independence breaks only for the one critique call, providers are clamped to the panel's first two (no reorder/reroute), and every governed panel stays at exactly 2 candidate + 1 adjudication calls. Fully neutral when the Governor is OFF (default) and on governor-on consensus/unknown turns — candidate and synthesis prompts stay byte-for-byte today, and the characterization + Oracle + judgment-poll goldens are unchanged. Completes the adaptive-panel work (audit P1 #4).

## 3.112.0
feat(quality): the multi-brain panel now opens for the turns that actually need plurality — not just keyword-risky ones. Panel admission used to be risk-only (`panelPolicy` + a high/critical risk classification), so a genuine decision, a reversible-but-risky change, or a turn the system is honestly **unsure** about would never get a second independent brain unless it happened to trip the risk keywords. Now, when the performance Governor is ON, it grants `panelAllowed` for `decide` / `risky` / `investigate` (genuinely low-confidence) task shapes **when the budget can actually seat it** — non-frugal mode, ≥2 authenticated vendors, and a `turnCallBudget` of 3 (two independent candidates + one synthesis). This composes as a strict **union** with today's risk-based admission: existing `always` and high/critical-risk panels still form exactly as before (the legacy plan is evaluated first), an explicit `panelPolicy: 'off'` remains an absolute veto, and an ordinary build never newly panels. The grant/refusal is recorded honestly in the allocation reasons. Fully neutral when the Governor is OFF (the default): orchestrate makes the exact same `planPanel` call with no override, so the selected plan, prompts, events, provider requests, and accept-gate path are byte-for-byte today — the characterization and Oracle goldens stay identical. No new task shape was invented (`investigate` reuses the existing `confidenceTooLowToAct`-derived shape), no metered lever was added, no budget is reserved, and `route()` / capacity ordering are untouched. First slice of the adaptive-panel work (audit P1 #4); the budget-honest conditional cross-critique round lands next.

## 3.111.0
feat(quality): the performance Governor now governs **every** execution path — one budget, no path can overspend. The Governor produces a single authoritative `AllocationPlan` (a hard 1–3 `turnCallBudget`, round budget, verify level, panel/hedge permission), but it was consulted *too late*: the panel and latency-hedge forks returned before the `allocate()` call ever ran, so the "harder" multi-brain turns got **no budget at all**, and `turnCallBudget` was never handed to any executor — the sequential loop was bounded only by `maxAttempts` + failover. Now allocation happens **once, before the panel/hedge fork**, and a single remaining-call budget is threaded into all three executors (sequential, panel, hedge). Authority is **restrictive-only** — it can deny or shrink a path, never newly enable one a current gate would refuse: the sequential loop stops escalating/failing-over once the budget is spent (taking the existing best-effort/loop-exhausted exit, which already passes through the 3.108 accept gate); a panel reduces its candidate count to fit one synthesis call and refuses to form if it can't seat the minimum 2+synthesis; a hedge refuses to form below a 2-call budget and caps the winner's repair. **Budget accounting is exact across failed/cancelled calls**: every provider invocation — main stream, review, critic, repair, and failover — consumes exactly one unit *when invoked*, regardless of success, error, or cancellation, so a turn can never multiply its way past the cap. A denied panel/hedge falls through to the sequential path and appends the user message exactly once (no double-append). Fully neutral when the Governor is OFF (the default): the allocation block is a pure relocation that does nothing, no budget is threaded, denials are impossible, and the orchestration characterization **and** Oracle goldens stay byte-identical (10/10 + 6/6). `route()` and the live capacity/provider ordering are untouched. With this, the elite-pro control loop — understand → allocate → execute → verify → repair-or-accept — has one authoritative allocation step that binds every path (audit P0 #2).

## 3.110.0
feat(quality): reasoning effort now sizes to the actual task, not just coarse buckets. Effort selection previously ignored signals the system already computes — it was static within mode × tier × risk × task-kind. Now `selectReasoningEffort` also considers the turn's engagement depth, plan-first, and (only from a real model intent extraction) the intent confidence and genuine-fork count, applying a bounded **±1 step**: raise effort on a genuinely deep (depth-2), low-confidence, plan-first, or multi-fork turn; lower it on a genuinely trivial one. The bump is clamped between `low` and the mode+tier hard-turn ceiling (cost-saver never raises on a hint — still never `xhigh`), and the existing capability step-down runs unchanged so an unsupported effort can never be requested. Hedge and panel candidates now use the turn's **real** task kind instead of a hard-coded `implementation`. Conservative by design and neutral on ordinary turns: depth defaults to neutral, and the rules-based intent placeholder (which reports `low` confidence for every IC/manager turn) is explicitly NOT treated as real uncertainty, so it can't inflate effort — only a genuine model extraction moves the dial. All orchestration characterization goldens stay byte-identical. Part of the elite-pro pipeline upgrades (audit P0 #3).

## 3.109.0
feat(quality): panel and hedge now pass through the same verify→repair→accept gate (3.108.0 parity). The cross-vendor panel and latency hedge previously returned their result *before* the quality gate — so the "harder" multi-brain turns actually skipped the verification/repair loop that sequential turns got. Now a successful panel synthesis and a hedge winner are both routed through the shared Candidate Quality Gate from 3.108.0: verify → on a red test or critic "revise", one bounded same-author repair (panel repairs on the synthesizer; hedge repairs only on the chosen winner — never reruns candidates, the loser, synthesis, or selection) → re-verify → accept only if green, else `success: false` with no persisted broken answer. Identical neutrality guarantee: a panel/hedge turn with no tests, passing tests, or no verify port behaves exactly as before (byte-identical transcripts); only genuine red-test/critic-revise turns change. All orchestration characterization goldens remain byte-identical. With this, every execution path — sequential, panel, and hedge — ends at one shared evidence gate; no path can declare success over a failing check.

## 3.108.0
feat(quality): a red test can no longer be reported as success — shared verify→repair→accept gate (sequential path). Previously, verification ran *after* the assistant turn was finalized and a failing test only produced a warning receipt — the turn still returned `success: true` and the answer was persisted. Now every successful sequential outcome (ordinary accept, reviewer-approved, and the best-effort fallback paths) passes through one **Candidate Quality Gate**: verify → classify (passing / failing / unverified) → if a real test came back **red** (or a reviewer critic explicitly asked to *revise*), run **one** bounded, evidence-scoped repair on the same provider (reusing the existing manager-notes retry — no new review/escalation loop) → re-verify once → accept only if it's now green; otherwise return `success: false`, surface the failure, and **do not append the known-broken answer to history**. Fail-soft is fully preserved: no tests, no detectable test command, an unavailable/timed-out runner, or a non-code turn all classify as *unverified* and accept exactly as before — only genuine negative evidence (a typed red test or a parsed critic "revise") changes behavior. `/goal` turns bypass this gate so their existing verified-done/fix-it loop stays the single authority (no double-verification). Reuses `verifyStage`, `managerNotes`, `MAX_REVISE_RETRIES`, and the existing append helper; adds at most one provider call, only after real failure evidence. Fully neutral otherwise: all existing orchestration characterization goldens stay byte-identical. (Panel and hedge paths route through the same gate next.) This is the first of the elite-pro pipeline upgrades — closing the loop so the system never declares done over a failing check.

## 3.107.0
chore(intensity): foundation for the Intensity dial (no behavior change yet). Lays the plumbing for an upcoming single user-facing control — **Intensity 1–5 + Auto** (1 Focused · 2 Pair · 3 Fleet · 4 Fleet+Hedge · 5 Max) — that will steer how aggressively myshell engages your provider fleet, with **Auto as the smart, plug-and-play default** (sizes effort to each turn; the dial is only for power-user tuning). This release ships the inert substrate only: the capacity model's intensity scale is finalized at 1–5 (mapping 1:1 to five real engagement regimes — no fake granularity), plus a global default (`AppConfig.intensity`) and a per-conversation override (`ConversationMeta.intensity` + `setIntensity`, canonicalized so Auto = absence and preserved across all conversation-metadata edits), and a pure precedence resolver (conversation > global > legacy mode > Auto). Nothing is wired into routing, engagement, or the UI yet — orchestration behavior is unchanged and the characterization suite stays byte-identical. The smart-Auto engagement wiring and the dial UI land next.

## 3.106.0
feat(orchestration): active capacity allocator — provider order now rebalances live with your usage. Building on the inventory weights from 3.104.0, myshell now tracks how many tokens each provider has handled **this conversation** (seeded from the ledger, updated as calls record) and orders providers by *weighted-fair load*: `normalizedLoad = sessionTokens ÷ capacityWeight`, lowest first. So your highest-capacity subscription (e.g. a Claude Max, weight 10) stays the workhorse and absorbs roughly 10× the work of a weight-1 plan before that lighter plan picks up a turn — your $200 sub leads, the $20 ones are genuine second/third brains, and traffic shifts off whatever you've leaned on. A rate-limited provider (already cooled down on a 429) is moved to the back regardless of weight — estimate to lead, react to be right. This composes with the optional outcome learner and rides the existing `learnedProviderOrder` seam, so **`route()` is unchanged**. Still no quota meter and no invented numbers — only relative weights, real local consumption, and real 429s. Neutral by construction: a fresh conversation with no recorded consumption (and no cooldown / outcome history) passes no dynamic order at all and behaves exactly as before — the orchestration characterization suite stays byte-identical. Cost accounting is untouched: the ledger is written first, the in-session tally updated only after a durable append, never double-counted. (UI dial to steer this — Intensity 1–10/Auto — lands next.)

## 3.105.0
fix(menu): refresh subscription/plan status on return to the menu (no more stale "ready (max_5x)"). The environment — each provider's signed-in state and plan tier — used to be detected once at startup and reused forever, so a plan change, a fresh login, or a new sign-in elsewhere never showed until you restarted. The menu now re-detects when you return to it, debounced by a 15-second TTL so it never spawns provider CLIs on every keypress/repaint: within the window it's a zero-cost no-op returning the cached snapshot, and a refresh in flight is de-duplicated. On the live-region (Ink) path the refresh runs in the background and repaints when it lands; on the legacy path it awaits only when stale, so output stays deterministic. Detection failures are fail-soft (the prior snapshot is kept — providers never blank out). Explicit login/install actions now force an immediate refresh through the same cache so the timestamp stays coherent. Scoped to the menu display; per-turn routing-freshness refresh arrives with the live allocator.

## 3.104.0
feat(orchestration): subscription-capacity substrate + inventory-driven provider order (foundation). Introduces a pure capacity model (`src/core/capacity-allocator.ts`) that classifies each authenticated provider's plan into a relative capacity *weight* — your highest-capacity subscription (e.g. a $200 Claude Max) becomes the workhorse, lighter plans ($20-tier) the second/third brains — without inventing absolute quota numbers (no meter exists for subscriptions; weights are relative only). The per-turn provider order is now derived from this inventory instead of a static claude-first list. Fully additive and **neutral by construction**: a single provider, or providers of equal/unknown plan, resolve to the exact canonical `claude → codex → opencode` order, so existing behavior — and the orchestration characterization suite — is byte-identical. The order only changes when a *non-Claude* provider has a strictly higher observed plan tier (e.g. Free Claude + observed Max Codex → Codex leads). Also lands the (not-yet-wired) pieces for the next steps: an Intensity dial (1–10/Auto → five honest regimes), a pure efficiency-first per-turn Auto-intensity mapping, and a weighted-fair live-order function (`normalizedLoad = sessionTokens ÷ weight`) that later slices feed live session consumption + 429 signals through the existing learned-routing seam — so `route()` itself never changes. Foundation only; the live allocator, intensity UI, and consumption tracking arrive in subsequent slices.
