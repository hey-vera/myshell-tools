...
- **One quality knob — `Efficient · Balanced · Max`** — sets *how high routing is allowed to reach*. Within any mode the router still escalates as a turn needs it, but the ceiling rises with the mode: **only `Max` opens the strongest (Opus / GPT‑5.5‑class) model.** `Efficient` and `Balanced` top out on the lighter and mid‑tier models — launching the most powerful (and priciest) model is a deliberate choice, not a silent default. **The default auto-detects from your subscription** (a Max plan auto-selects `Max` and opens the Opus tier; smaller plans default to `Balanced`) — no configuration, no interrogation. Change it on the home screen (`m`), in-chat (`/mode`), or in Settings; it's one global knob, changeable anywhere.
  - `Efficient` — stays on the lighter models, escalating among them only when a turn needs it; reviews cross-vendor on *critical*-risk tasks only. Won't open the top model.
  - `Balanced` — escalates to the mid-tier model on harder turns; the strongest model stays reserved for `Max`.
