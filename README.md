...
> **Status: tip `3.173.0` (GitHub `main`) — external-ship wave in progress.** Multi-goal one-chat over subscription CLIs (Claude / Codex / Grok / OpenCode; OpenCode Go key is the only intentional key exception). **npm `latest` may lag this tip until owner publish** — prefer `git`/`npm link` for tip behavior, or check `npm view myshell-tools version`. Effort (mode) + Speed (multi-goal concurrency), Shift+Tab per-conversation Effort, Esc exits, `b` back, `c` panel, durable drafts/resume, goals survive leave-chat, daemon-lite workers (free-loop ≤8 + fenced leases), per-account model probe on menu path, accounts list nav/rename/mouse, forge-aware PR/MR when `gh`/`glab` present. Ship checklist: `docs/EXTERNAL-READINESS-PLAN.md`.
