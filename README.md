...
> **Status: tip `3.174.0` (GitHub `main`) — code gate for external ship; owner smoke + publish remaining.** Multi-goal one-chat over subscription CLIs (Claude / Codex / Grok / OpenCode; OpenCode Go key is the only intentional key exception). **npm `latest` may lag until owner publish** — check `npm view myshell-tools version` (was **3.170.0** at cut). Effort (mode) + Speed (multi-goal concurrency only), Shift+Tab, Esc exit, `b`/`c`, durable drafts/resume, leave-chat goals, daemon-lite workers (free-loop ≤8 + fenced leases + **account enrich parity**), per-account probe, accounts list honesty, `smoke:multichat` + `smoke:packed`. Ship checklist: `docs/EXTERNAL-READINESS-PLAN.md`.
