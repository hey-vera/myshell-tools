...
**Measurable done:** user can run a single Replit/local session and complete: change effort (box updates), open chat, see clear legend, cycle mode with Shift+Tab, open/close control panel without stuck state, type a message without eternal GǣThinkingGǪGǥ, see one goals surface + bottom recap, resume with partner addressing inactive goals, kill is not required for lag recovery.
...
| Mode list keys | Display order is Budget=1 GǪ Auto=5; **keys must match** (`ALL_LEVELS` order). |
...
| Thinking | State machine: prep phases honest (`Resuming` / `Routing` / GǪ); **Thinking** only when model is composing; then `Responding` / tool verbs. No eternal Thinking on submit. |
...
- [x] **P0.2** Fix mode key mapping `[1]→�[5]` to match `ALL_LEVELS` (BudgetGǪAuto)
...
- [ ] **P0.9** Empty-buffer `→�` opens control panel; `→�` back to menu (focus model documented + tested)
