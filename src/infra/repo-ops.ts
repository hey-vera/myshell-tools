...
/**
 * Shared parser for `git status --porcelain` (and =v1) output.
 * Returns repo-relative POSIX paths (defensively normalized).
 * Used by LocalRepoOps and re-exported for use by verify-port / repo-scan
 * to avoid duplicated status parsing logic.
 */
export function parsePorcelain(raw: string): readonly string[] {
  return raw
    .split(/\r?\n/)
