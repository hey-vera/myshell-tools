...
function isGrokAccount(a: SubscriptionAccount): a is GrokSubscriptionAccount {
  return a.provider === 'grok' && subscriptionAccountKind(a) === 'oauth-sub';
}
...
async function createGrokAccountFlow(
  out: OutputSink,
  ...
  const existingGrokOauth = file.accounts.filter(isGrokAccount);

  // macOS guard: block a 2nd grok oauth account on darwin
  if (platform === 'darwin' && existingGrokOauth.length >= 1) {
    out.write(
      yellow(
        'Grok OAuth accounts cannot be isolated on macOS because Grok ' +
        'stores OAuth tokens in a shared Keychain service. Keep the existing ' +
        'Grok account or delete it before creating another.',
        out.color,
      ) + '\n',
    );
    return;
  ...
  const defaultLabel = `Grok ${existingGrokOauth.length + 1}`;
  const account = newGrokAccount({
    id: safeId,
    label: defaultLabel,
    nowIso: clock.isoNow(),
  });
  ...
  // Run Grok OAuth login with account-scoped env
  out.write(bold('\nStarting Grok sign-in for this account...\n', out.color));
  await login(out, 'grok', {
    readLine,
    confirm,
    ...(suspendStdin !== undefined ? { suspendStdin } : {}),
    accountEnv: { GROK_HOME: account.homeDir },
  });
  ...
  if (detection.status !== 'active') {
    out.write(
      yellow(
        'Grok sign-in did not authenticate with this account. ' +
  ...
  const finalAccount: GrokSubscriptionAccount = {
    ...account,
    label,
    status: 'active',
    plan: detection.plan,
...
async function editAccountScreen(
  ...
  for (;;) {
    ...
    out.beginFrame?.();
    out.write(`\n${bold('Edit Grok Account: ' + account.label, out.color)}\n\n`);
    out.write(`  provider: grok\n`);
    ...
    if (key === 'l') {
      ...
      out.write(bold('\nRe-authenticating Grok account...\n', out.color));
      await login(out, 'grok', {
        readLine,
        confirm,
        ...(suspendStdin !== undefined ? { suspendStdin } : {}),
...
export async function runGrokAccountsMenu(
  out: OutputSink,
  readLine: () => Promise<string | null>,
  confirm: Confirm,
  clock: Clock,
  ...
  for (;;) {
    ...
    const accounts = allAccounts.filter(isGrokAccount);
    selectedIndex = moveListHighlight(selectedIndex, 0, accounts.length);

    out.beginFrame?.();
    out.write('\n' + bold('Grok Accounts', out.color) + '\n');
