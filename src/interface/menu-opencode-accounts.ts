...
async function createAccountFlow(
  ...
  if (label.length === 0) {
    let subsFile: { version: 1; accounts: readonly OpencodeSubscriptionAccount[] };
    try {
      subsFile = await readSubscriptions();
    } catch {
      subsFile = { version: 1, accounts: [] };
    }
    const samePool = subsFile.accounts.filter((a) => a.pool === pool).length;
...
export async function runOpencodeAccountsMenu(
  ...
  for (;;) {
    let accounts: readonly OpencodeSubscriptionAccount[];
    try {
      const file = await readSubscriptions();
