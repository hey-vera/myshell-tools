 ...
 * `npm test` suite under strip-types. DEFAULT OFF — this behavior-changing
 * feature changes ZERO routing unless the caller explicitly opts IN via
 * `MYSHELL_AUTO_BRAIN` ∈ {'1','true','on','yes'} (case-insensitive, trimmed) or
 * persisted `config.experimentalAutoBrain === true`. Mirrors level-flag.ts /
 * verify-flag.ts. Rollback forces it OFF (kill-switch parity).
 *
...
/**
 * Decide whether the Auto Brain is enabled. DEFAULT FALSE. Returns true ONLY on
 * an explicit opt-IN: `MYSHELL_AUTO_BRAIN` ∈ {'1','true','on','yes'} (trimmed,
 * case-insensitive) OR `config.experimentalAutoBrain === true`. Rollback forces
 * it off. Any other value (including absent, '0', 'false', '') → false. Never
 * throws.
 */
