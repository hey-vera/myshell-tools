...
export function buildUnderstandingPrompt(task: string, repoContext?: string): string {
  ...
  const lines: string[] = [
    ...
    'engineering partner. BEFORE any goals are planned, your job is to gain DEEP,',
    'EXPERT, CURRENT understanding of the REAL system the work touches — so the plan',
    'reflects the whole motherboard, not a naive parts-list.',
    '',
    'This is a READ-ONLY investigation. You MAY read files to understand the system.',
    'You MUST NOT modify anything. Map the RELEVANT part of the real codebase:',
    ...
    '  - the genuinely-OPEN questions you CANNOT resolve from the code — the things a',
    '    senior would have to ASK the owner before committing (scale? quality bar?',
    '    which surface? a destructive choice?). These become clarifying questions.',
    '',
    ...
    '  - Ground everything in the ACTUAL code you read. Do not guess at modules or',
    '    constraints you did not verify.',
  ...
  if (typeof repoContext === 'string' && repoContext.trim().length > 0) {
    lines.push('', 'REPOSITORY ORIENTATION (the real tree — read the relevant files):', repoContext.trim());
  }
