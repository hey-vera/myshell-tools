...
export function buildRouterPrompt(task: string): string {
  return [
    ...
    '',
    'tier:',
    '  "worker"  — a simple read/lookup/explain/summary; no file changes.',
    '  "ic"      — implement, edit, debug, or refactor within one area.',
    '  "manager" — architecture, audits, multi-system or cross-cutting design,',
    '              comparing approaches, or high-stakes planning.',
    'plan: true if the task is complex or multi-step enough that proposing a',
    '  short plan before acting would help; otherwise false.',
    '',
    'Reply with ONLY a JSON object on a single line, nothing else:',
    '{"tier":"ic","plan":false,"reason":"<8 words max>"}',
    '',
    `Message: ${task}`,
  ].join('\n');
}
