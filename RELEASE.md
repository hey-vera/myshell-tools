...
The implementation for **3.2.0** is complete and green on the
...
`package.json`'s `version` first if 3.2.0 is ever taken. After publishing, the
`latest` tag moves from 3.0.0 → 3.2.0 and the in-tool auto-update can reach
existing users.
...
myshell-tools --version   # expect 3.2.0
```
